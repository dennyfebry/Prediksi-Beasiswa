...
for ($i = 2; $i <= $jumlah_baris; $i++) {
    ...
    if ($npm != "" && $nama != "" && $jenis_kelamin != "" && $tgl_lahir != ""  && $angkatan != "" && $fakultas != "" && $prodi != "" && $periode != "" && $ipk != ""  && $sks_lulus != "" && $penghasilan_orangtua != "" && $tanggungan != "" && $status != "") {
        ...
        mysqli_query($connect, "INSERT into training values('','$npm', '$nama', '$jenis_kelamin', '$tgl_lahir', '$angkatan', '$fakultas', '$prodi', '$periode', '$ipk',  '$sks_lulus', '$penghasilan_orangtua', '$tanggungan', '$status', '$tags')");
        // $berhasil++;
    }
}

// hapus kembali file .xls yang di upload tadi
unlink($_FILES['filetraining']['name']);
